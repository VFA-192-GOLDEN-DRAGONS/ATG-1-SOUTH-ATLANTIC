...
-- local DCSWorker = nil
-- --check if file exists, and load it onlycom then
-- if lfs.attributes(lfs.writedir()..[[Scripts\DCSWorkerEvents.lua]]) then
    -- DCSWorker=dofile(lfs.writedir()..[[Scripts\DCSWorkerEvents.lua]])
-- end
...
csar.csarUnits = {} -- List of all the MEDEVAC _UNIT NAMES_ (the line where it says "Pilot" in the ME)!
...
--this function needs moose
function getCSARunits()
	local lastunit = #csar.csarUnits + 1

	for name,client in pairs(_DATABASE.CLIENTS) do
		if string.find(name, "SAR") or string.find(name, "Helo") or string.find(name, "HELO") or string.find(name, "Auto%-Lase") then
			csar.csarUnits[lastunit] = name
			lastunit = lastunit + 1
			env.info("Found CSAR unit: "..name)
		end
	end
...
csar.addBeaconToGroup = function(_woundedGroupName, _freq)
    ...
    local _sound = "TTI Soundfiles/" .. csar.radioSound
...
air_support_csar_circle_id = {}
air_support_csar_text_id = {}
...
function csar.spawnGroup(_deadUnit)
	...
	--[[
	local csarSupportRequestText = "Requesting EVAC:\n" .. _groupName
	airSupportCsar_coord = COORDINATE:NewFromVec3(_pos)
	air_support_csar_circle_id[_id] =  UTILS.GetMarkID()
	air_support_csar_circle_id[_id] = airSupportCsar_coord:CircleToAll(tti.marker.airsupport.circleSize, -1, tti.marker.airsupport.circleOutlineColor, 0.5, tti.marker.airsupport.circleFillColor, tti.marker.airsupport.circleFillAlpha, 3, false)
	air_support_csar_text_id[_id] = UTILS.GetMarkID()
	air_support_csar_text_id[_id] = airSupportCsar_coord:Translate(tti.marker.airsupport.textBoxOffset, tti.marker.airsupport.textBoxAnglePosition):TextToAll(csarSupportRequestText, -1, tti.marker.airsupport.textBoxTextColor, tti.marker.airsupport.textBoxTextAlpha, tti.marker.airsupport.textBoxFillColor, tti.marker.airsupport.textBoxFillAlpha, tti.marker.airsupport.textBoxFontSize, false)
	--]]
...
function csar.pickupUnit(_heliUnit, _pilotName, _woundedGroup, _woundedGroupName)
    ...
    trigger.action.outText(Unit.getByName(_heliName):getPlayerName() .. " has picked up " .. _pilotName, 20)
    csar.displayMessageToSAR(_heliUnit, string.format("%s: %s I'm in! Get to a FARP, BASE or SHIP ASAP! ", _heliName, _pilotName), 10)
...
function csar.rescuePilots(_heliUnit)
    ...
    trigger.action.outText(Unit.getByName(_heliUnit:getName()):getPlayerName() .. " has completed a search and rescue operation. Excellent work! ", 20)
	
	if DCSWorker ~= nil then --track CSAR stat
    DCSWorker.sendCSAREvent(Unit.getByName(_heliUnit:getName()):getPlayerName()); 
	env.info('CSAR Worker EVENT Logged' .. Unit.getByName(_heliUnit:getName()):getPlayerName() .. ': +1 CSAR rescue')
	end

 --   trigger.action.outText(Unit.getByName(_heliUnit:getName()):getPlayerName() .. " has completed a search and rescue operation. Excellent work! ", 20)
    trigger.action.outSound("TTI Soundfiles/Morse code UI.ogg")
...
function csar.delayedHelpMessage(_args)
    local status, err = pcall(function(_args)
        ...
        if _heli ~= nil and #csar.getWoundedGroup(_injuredGroupName) > 0 then
            ...
            if _groupId then
                trigger.action.outSoundForGroup(_groupId, "TTI Soundfiles/CSAR.ogg")
            end
...
function csar.checkOnboard(_unitName)
    ...
    if _inTransit == nil or csar.tableLength(_inTransit) == 0 then
        ...
        local _text = "Onboard - RTB to FARP/Airfield/Ship: "
        ...
        if _unit ~= nil then
            ...
            if _groupId then
                ...
                if csar.addedTo[tostring(_groupId)] == nil then
					...
					missionCommands.addCommandForGroup(_groupId, "-Force Start CSAR Mission-", _rootPath, csar_mission_start, _unitName)
...
env.info("TTI: CSAR event handler added!")
...
--trigger.action.outText("CSAR Script (Helo Rescue Script) loaded!", 15)
