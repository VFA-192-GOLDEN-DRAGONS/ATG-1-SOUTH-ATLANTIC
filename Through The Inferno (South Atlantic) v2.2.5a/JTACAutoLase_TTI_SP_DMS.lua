...
mission_setup_time_passed = false
mission_setup_time_length = 300


timer.scheduleFunction(function() --Supress JTAC Messages during setup time.
	mission_setup_time_passed = true
end, nil, timer.getTime() + mission_setup_time_length )
...
function JTACAutoLase(jtacGroupName, laserCode,smoke,lock,colour)
  ...
  --      env.info("TEMP UNIT INFO: " .. tempUnitInfo.name .. " " .. tempUnitInfo.unitType)
        ...
        if tempUnit ~= nil and tempUnit:isActive() == true and mission_setup_time_passed == true then
    ...
    if enemyUnit == nil then
        ...
        if enemyUnit ~= nil then
			...
			if mission_setup_time_passed == true then
...
--    end
--end
--trigger.action.outText("JTAC Auto-Lase LUA file loaded...", 10)
