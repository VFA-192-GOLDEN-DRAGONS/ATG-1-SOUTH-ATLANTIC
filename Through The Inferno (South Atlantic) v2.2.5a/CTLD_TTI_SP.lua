...
ctld.hoverTime = crate_pickup_hover_time -- Time to hold hover above a crate for loading in seconds
...
ctld.pickupZones = {
    { "pickzone1", troop_pickup_smoke_color, -1, troop_pickup_smoke, 0 },
    { "pickzone2", troop_pickup_smoke_color, -1, troop_pickup_smoke, 0 },
    { "pickzone3", troop_pickup_smoke_color, -1, troop_pickup_smoke, 0 },
    { "pickzone4", "none", -1, troop_pickup_smoke, 0 },
    { "pickzone5", "green", -1, "yes", 0 },
    { "pickzone6", "green", -1, "yes", 0 },
    { "novoro_pickzone", "green", -1, "yes", 2 },
    { "nalchik_pickzone", "green", -1, "yes", 0 },
    { "pickzone7", "green", -1, "yes", 0 },
    { "pickzone8", "green", -1, "yes", 0 },
    { "pickzone10", "green", -1, "yes", 0 },  -- limits pickup zone 10 to 10 groups of soldiers or vehicles, only blue can pick up

    { "pickzone11", "white", 20, "no", 2 },  -- limits pickup zone 11 to 20 groups of soldiers or vehicles, only blue can pick up. Zone starts inactive!
    { "pickzone12", "white", 20, "no", 1 },  -- limits pickup zone 11 to 20 groups of soldiers or vehicles, only blue can pick up. Zone starts inactive!
    ...
    { "USA SHIP", "blue", 10, "yes", 0, 1001 }, -- instead of a Zone Name you can also use the UNIT NAME of a ship
}
...
function getCTLDunits()
	local lastunit = #ctld.transportPilotNames + 1
	...
	for name,client in pairs(_DATABASE.CLIENTS) do
		if string.find(name, "SAR") or string.find(name, "Helo") or string.find(name, "HELO") or string.find(name, "Auto%-Lase") then
			ctld.transportPilotNames[lastunit] = name
			lastunit = lastunit + 1
			env.info("Found CTLD unit: "..name)
		end
	end
...
ctld.logisticUnits = {
    ...
    "logistic13",
    "logistic14",
	"Static FARP Ammo Storage-1",
	"Static FARP Ammo Storage",
}
...
if slingLoad == true then
trigger.action.outText("CTLD crate loading mode: Real Sling Loading", 10)
else
trigger.action.outText("CTLD crate loading mode: Hover Loading", 10)
end
