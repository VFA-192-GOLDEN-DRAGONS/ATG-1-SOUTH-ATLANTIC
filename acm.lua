...
BFMACM = {
  menuAdded = {},
  menuF10 = {},
  zoneBfmAcmName = "ACM", -- The BFM/ACM Zone
  zonesNoSpawnName = { -- zones inside BFM/ACM zone within which adversaries may NOT be spawned.
      "zone_box",
  },
  adversary = {
    menu = { -- Adversary menu
      {template = "ADV_F1CE", menuText = "Adversary F1"},
      {template = "ADV_M2000", menuText = "Adversary Mirage V"},
      {template = "ADV_A4", menuText = "Adversary A4"},
      {template = "ADV_F5", menuText = "Adversary F-5"},
      ...
      {template = "ADV_F14", menuText = "Adversary F-14"},
    },
...
if _zone == nil then
  _msg = "[ACM] ERROR: ACM Zone: " .. tostring(BFMACM.zoneBfmAcmName) .. " not found!"
  BASE:E(_msg)
else
  BFMACM.zoneBfmAcm = _zone
  _msg = "[ACM] ACM Zone: " .. tostring(BFMACM.zoneBfmAcmName) .. " added."
  BASE:T(_msg)
end
...
for i, adversaryMenu in ipairs(BFMACM.adversary.menu) do
  ...
  if _adv then
    ...
    _msg = "[ACM] ERROR: spawn template: " .. tostring(adversaryMenu.template) .. " not found!" .. tostring(zoneNoSpawnName) .. " not found!"
    BASE:E(_msg)
  end
end
...
function BFMACM.SpawnAdv(adv,qty,group,rng,unit)
  ...
  local msgNoSpawn = ", Cannot spawn adversary aircraft if you are outside the ACM zone!"
  ...
  if spawnAllowed then
    ...
    msgNoSpawn = ", Cannot spawn adversary aircraft outside the ACM zone. Change course and try again."
  end
    ...
    if group and gid then
      ...
      if BFMACM.menuAdded[uid] == nil then
        ...
        if BFMACM.menuF10[gid] == nil then
          ...
          BFMACM.menuF10[gid] = MENU_GROUP:New(group, "AI ACM")
        end
        if BFMACM.menuF10[gid][uid] == nil then
          ...
          BASE:T("[ACM] Add submenus and range selectors for player: " .. playername)
